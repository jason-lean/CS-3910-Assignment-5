...
<head>
	<title>
		<?php echo "User Registration - " . $Title; ?>
	</title>
...
//	print_r($_POST);
	...
	if (!$email) {
		punt("Please enter an e-mail address.");
	}
	...
	if (!$password2) {
		punt("Please re-enter your password.");
	}
	...
	// set up my query
	$query = "SELECT email FROM Users_T WHERE email='$email';";
	...
	// set up my query
	$query = "INSERT INTO Users_T(email, hashedPass) VALUES ('$email', '$hashedPass');";
...
<?php
	...
	// set up my query
	$query = "SELECT email FROM Users_T ORDER BY email;";
